...
-- Description: Creates the partners table with SINGLE_TABLE inheritance for B2B, B2C, and SUPPLIER partners
-- Version: 1.2.0
-- Date: 2024-01-15
...
CREATE TABLE partners (
    ...
    partner_type VARCHAR(10) NOT NULL CHECK (partner_type IN ('B2B', 'B2C', 'SUPPLIER')),
    ...
    minimum_order_amount DECIMAL(24,6),
    lead_time_days INTEGER,
    certification_iso VARCHAR(100),
    supplier_since TIMESTAMP WITH TIME ZONE,
...
CREATE INDEX idx_partners_supplier_code ON partners(supplier_code);
CREATE INDEX idx_partners_supplier_category ON partners(supplier_category);
CREATE INDEX idx_partners_supplier_status ON partners(supplier_status);
CREATE INDEX idx_partners_next_audit_date ON partners(next_audit_date);
...
CREATE INDEX idx_partners_supplier_performance ON partners(delivery_performance_score, quality_score, price_competitiveness_score);
...
ALTER TABLE partners ADD CONSTRAINT chk_supplier_scores CHECK (delivery_performance_score >= 0 AND delivery_performance_score <= 100);
ALTER TABLE partners ADD CONSTRAINT chk_quality_score CHECK (quality_score >= 0 AND quality_score <= 100);
ALTER TABLE partners ADD CONSTRAINT chk_price_competitiveness_score CHECK (price_competitiveness_score >= 0 AND price_competitiveness_score <= 100);
ALTER TABLE partners ADD CONSTRAINT chk_minimum_order_amount_positive CHECK (minimum_order_amount >= 0);
ALTER TABLE partners ADD CONSTRAINT chk_lead_time_days_positive CHECK (lead_time_days >= 0); 